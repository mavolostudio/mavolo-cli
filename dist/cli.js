...
async function promptForTemplate(options) {
    const templateChoices = options.projectType === "web"
...
async function promptForLibraries(options, selectedTemplate) {
    const libraries = projectTemplates[options.projectType][0][selectedTemplate].library;
...
program
    ...
    const selectedLibraries = await promptForLibraries(options, selectedTemplate);
